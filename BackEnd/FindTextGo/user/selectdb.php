...
// 특정 사용자의 파일 목록을 가져오는 함수
function getFileList($conn, $userId)
{
	$sql = "SELECT f.file_id, f.file_hash, f.file_extension, f.file_size, f.pdf_page_count, f.image_processed_count, u.file_name 
			FROM file_info f
	...
	$stmt->bind_param("i", $userId); // user_id를 필터링하여 해당 사용자의 파일만 조회
	$stmt->execute();
	$result = $stmt->get_result();
